...
| Antigravity desktop black/blank window | Electron GPU renderer failed during startup | Use the desktop `Antigravity.lnk`; it launches `Antigravity.exe` directly with `--disable-gpu`, without a PowerShell window. Startup diagnostics are in `data/antigravity-electron.log`. |
