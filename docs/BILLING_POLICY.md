...
- **允许付费 API**：DeepSeek（承担批量低成本工作，每月硬闸 30 元人民币）。
...
## 2. DeepSeek 每月 30 元硬闸体系
- **共享一本账 (Single Ledger)**：Hermes（当使用 DeepSeek provider 时）与 DeepSeek 必须计入同一本月度账本 (`data/billing/deepseek-YYYY-MM.json`)，不得另开口子，Hermes 绝非免费通道。
- **自然月硬闸**：每月上限 30 元人民币 (`monthly_limit_cny: 30`)，按自然月重置。
