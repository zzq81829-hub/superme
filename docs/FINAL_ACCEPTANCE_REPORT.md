...
| T2 Hermes→Claude review | **AUTH_REQUIRED / SUBSCRIPTION_UNAVAILABLE** | `claude auth status` is `loggedIn` via **API key**, not subscription OAuth. Billing policy forbids ANTHROPIC_API_KEY. Need `claude auth login` (subscription). Not faked. |
